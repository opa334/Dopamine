...
__attribute__((constructor)) static void initializer(void)
{
	...
	if (comingFromUserspaceReboot) {
		...
		FCHandler *handler = [[FCHandler alloc] initWithReceiveFilePath:fakeRootPath(@"basebin/.communication/boomerang_to_launchd") sendFilePath:fakeRootPath(@"basebin/.communication/launchd_to_boomerang")];
		...
		[[NSFileManager defaultManager] removeItemAtPath:fakeRootPath(@"basebin/.communication") error:nil];
		if (boomerangPid != 0) {
			int status;
	...
	setenv("DYLD_INSERT_LIBRARIES", fakeRootPath(@"basebin/launchdhook.dylib").fileSystemRepresentation, 1);

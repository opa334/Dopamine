...
void libjailbreak_initialize_static_offsets(void)
{
	struct utsname name;
	uname(&name);
	...
	gSystemInfo.kernelStruct.ipc_space.table_entry_size = 0x18;
	...
	if (strcmp(xnuVersion, "21.0.0") >= 0) { // iOS 15+
		...
		if (strcmp(xnuVersion, "21.2.0") >= 0) { // iOS 15.2+
			...
			gSystemInfo.kernelStruct.proc.ucred = 0x0;
			gSystemInfo.kernelStruct.proc.csflags = 0x0;

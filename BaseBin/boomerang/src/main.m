...
int main(int argc, char* argv[])
{
	...
	dispatch_source_set_event_handler(serverSource, ^{
		...
		if (!xpc_pipe_receive(serverPort, &xdict)) {
			jbserver_received_xpc_message(&gBoomerangServer, xdict);
		}
	});
	dispatch_resume(serverSource);

	// When spawning, launchd should have stored a port to it's server in boomerang's registeredPorts[0]
	// Initialize jbclient with that
	mach_port_t *registeredPorts;
	mach_msg_type_number_t registeredPortsCount = 0;
	if (mach_ports_lookup(mach_task_self(), &registeredPorts, &registeredPortsCount) != 0 || registeredPortsCount < 0) return -1;
	jbclient_xpc_init_from_port(registeredPorts[0]);

	// Stash our server port inside launchd's registeredPorts[0]
	...
	kr = mach_ports_register(launchdTaskPort, (mach_port_t[]){ serverPort }, 1);

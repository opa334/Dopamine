...
int64_t initEnvironment(void)
{
	bool jbEnvironmentHidden = ![[NSFileManager defaultManager] fileExistsAtPath:@"/var/jb"];

	if (!jbEnvironmentHidden) {
		int unmountRet = unmountFakeLibBindMount();
		if (unmountRet != 0) {
			return 1;
		}
	}
	...
	if (!jbEnvironmentHidden) {
		int bindMountRet = mountFakeLibBindMount();
		if (bindMountRet != 0) {
			return 9;
		}
	}
		...
		if (messageType == XPC_TYPE_DICTIONARY) {
			...
			if (!systemwide || isAllowedSystemWide) {
				switch (msgId) {
					...
					case JBD_SET_FAKELIB_MOUNT_ACTIVE: {
						int64_t result = 0;
						if (gPPLRWStatus == kPPLRWStatusInitialized && gKCallStatus == kKcallStatusFinalized) {
							bool setActive = xpc_dictionary_get_bool(message, "active");
							if (setActive) {
								result = mountFakeLibBindMount();
							}
							else {
								result = unmountFakeLibBindMount();
							}
							NSLog(@"setActive: %d, result:%lld", setActive, result);
						}
						else {
							result = JBD_ERR_PRIMITIVE_NOT_INITIALIZED;
						}
					...
					case JBD_UNMOUNT: {
						...
						if (gPPLRWStatus == kPPLRWStatusInitialized && gKCallStatus == kKcallStatusFinalized) {
							...
							uint32_t flags = xpc_dictionary_get_uint64(message, "flags");
							if (path) {
								result = kernel_unmount(path, flags);
							}
						}
						else {
							result = JBD_ERR_PRIMITIVE_NOT_INITIALIZED;
